"""
pages/4_Simulateur_Infaisabilite.py — Infeasibility "What-If" simulator.

Consolidated and polished version (Phase 2/3 wrap-up):
- Auto-loads the latest real run data with a freshness indicator.
- Shows an at-a-glance summary first (feasibility, bottlenecks, unplaced).
- Automatic suggestions come FIRST when infeasibility is detected, followed
  by manual exclusion and extra-resource scenarios.
- Positive status and exploratory mode when the latest run is feasible.
...
if _src:
    st.caption(
        f"Source des donnees : {os.path.relpath(_src, _ROOT) if _src.startswith(_ROOT) else _src} "
...
# ---------------------------------------------------------------------------
# Section 1 — Automatic suggestions (first: most relevant on infeasibility)
# ---------------------------------------------------------------------------
